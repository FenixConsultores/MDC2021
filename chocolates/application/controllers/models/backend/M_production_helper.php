...
class M_production_helper extends CI_Model
{
	...
	public function getProductsMaterials()
	{
		$query = $this->db->query("
		SELECT 
			des.id_producto, psi.imagen, des.t_fresa, des.no_fresas, des.no_fresas_cafe, des.no_fresas_blanca,
			des.no_rosas, des.espirales, des.puntos, des.rayas, des.figuras, des.lisa, 
			des.s_chocolate, des.desnuda_rayada, des.rosa_de_chocolate, des.palo_chico, des.palo_grande
		FROM productos_materiales des 
		left join products_single_image psi on (des.id_producto = psi.id_producto)
		");
	...
	public function getProductMaterials($productId)
	{
		...
		SELECT 
			prod.id_producto, psi.imagen, des.t_fresa, des.no_fresas, des.no_fresas_cafe, des.no_fresas_blanca,
			des.no_rosas, des.espirales, des.puntos, des.rayas, des.figuras, des.lisa, 
			des.s_chocolate, des.desnuda_rayada, des.rosa_de_chocolate, des.palo_chico, des.palo_grande
		FROM productos prod 
		left join products_single_image psi on (prod.id_producto = psi.id_producto)
		left join productos_materiales des on (prod.id_producto = des.id_producto) 
		where prod.id_producto = $productId
		");
	...
	public function saveProductMaterials($data)
	{
		//begins transaction
		$this->db->trans_begin();
		...
		try {
			...
			//deletes product material row
			$this->db->query("delete from productos_materiales where id_producto = $productId");

			//re-inserts product material row

			$this->db->query("
			INSERT INTO productos_materiales (
				id_producto, t_fresa, no_fresas, no_fresas_cafe, no_fresas_blanca,
				no_rosas, espirales, puntos, rayas, figuras, lisa, 
				s_chocolate, desnuda_rayada, rosa_de_chocolate, palo_chico, palo_grande
			)
			VALUES ($productId,'$data[tStrawberry]',$data[nStrawberry],$data[nStrawberryBrown],$data[nStrawberryWhite],
					$data[nRoses],$data[nSpirals],$data[nDots],$data[nStripes],$data[nFigures],$data[nFlats],
					$data[nChocolate],$data[nNakedStripes],$data[nChocolateRose],$data[nSmallStick],$data[nBigStick])
			");
